...
export function buildAdPrompt(input: AdInput): string {
  const { listing } = input;
  // Inzertní voicebot (0800, ElevenLabs). Per-listing contactPhone má přednost,
  // jinak sdílené 0800 číslo, na které zájemci volají zdarma.
  const phone = listing.contactPhone ?? process.env.VOICEBOT_PHONE ?? "+420 800 881 993";
